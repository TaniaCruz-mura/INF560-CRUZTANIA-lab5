        ...
        <div class="lg:col-span-2 space-y-4">
            ...
            <div class="bg-slate-100 rounded p-4">
                ...
                <p class="text-slate-700">
                    <span class="text-2xl font-bold text-slate-900">{{ $book->available_copies }}</span>
                        ...
                        <tbody>
                            @foreach($book->activeLoans as $loan)
                                <tr class="border-b border-slate-200">
                                    <td class="px-3 py-2">{{ $loan->member->user->name ?? '—' }}</td>
                                    <td class="px-3 py-2">{{ $loan->loaned_at?->format('d/m/Y') }}</td>
                                    <td class="px-3 py-2">{{ $loan->due_at?->format('d/m/Y') }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
